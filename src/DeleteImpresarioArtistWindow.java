...
public class DeleteImpresarioArtistWindow extends JFrame {
    ...
    private void initImpresarioSelectPanel() {
        ...
        impresarioSelectPanel.add(impresarioSelector.getPanel(), BorderLayout.CENTER);
    ...
    public DeleteImpresarioArtistWindow() {
        try {
    ...
    public void openArtistSelectPanel() {
        ...
        String sql = "SELECT id, last_name, first_name, surname FROM artist JOIN work_with ON artist.id = work_with.artist_id WHERE impresario_id = ?";
    ...
    private void delete() {
        String preparedSQL = "DELETE FROM work_with WHERE impresario_id = ? AND artist_id = ?";
        try (Connection connection = DriverManager.getConnection(ConnectionConfig.url, ConnectionConfig.username, ConnectionConfig.password);
